...
namespace Post.CMD.Api.Controllers
{
    ...
    public class RemoveCommentController : ControllerBase
    {
        ...
        public async Task<ActionResult> RemoveCommentAsync(Guid id, RemoveCommentCommand command)
        {
            ...
            {
                _logger.Log(LogLevel.Warning, ex, "Could not retrieve aggregate, client passed an incorrect post Id target the aggregate!");
                return BadRequest(new BaseResponse
                {
                    Message = ex.Message,
                });
            ...
            {
                const string SAFE_ERROR_MESSAGE = "Erro while processing request to Remove a comment from a post!";
